...
namespace RT.Util
{
    sealed class PriorityQueue<TItem, TWeight> where TWeight : IComparable<TWeight>
    {
        ...
        public PriorityQueue(bool largestFirst)
        {
            _elements = new TItem[64];
            _weights = new TWeight[64];
            _count = 0;
            _smallestFirst = largestFirst;
        }

        private int compare(int a, int b)
        {
            return _smallestFirst ? _weights[a].CompareTo(_weights[b]) : _weights[b].CompareTo(_weights[a]);
        }
