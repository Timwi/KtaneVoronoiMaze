...
namespace RT.KitchenSink.Geometry
{
    ...
    public sealed class VoronoiDiagram
    {
        ...
        private sealed class data
        {
            ...
            public data(PointD[] sites, double width, double height, VoronoiDiagramFlags flags)
            {
                ...
                for (var siteIx = 0; siteIx < sites.Length; siteIx++)
                {
                    ...
                    if (p.X > 0 && p.Y > 0 && p.X < width && p.Y < height)
                        SiteEvents.Add(new siteEvent(new site(siteIx, p)));
                ...
                {
                    while (i < SiteEvents.Count && SiteEvents[i - 1].Site.Position == SiteEvents[i].Site.Position)
                    {
                        if ((flags & VoronoiDiagramFlags.RemoveDuplicates) == VoronoiDiagramFlags.RemoveDuplicates)
                            ...
                            throw new Exception("The input contains two points at the same coordinates " +
                                SiteEvents[i].Site.Position + ". Voronoi diagrams are undefined for such a situation. " +
                ...
                {
                    if (CircleEvents.Count > 0 && (SiteEvents.Count == 0 || CircleEvents[0].X <= SiteEvents[0].Site.Position.X))
                    {
                        ...
                        if (arcIndex > 0)
                            checkCircleEvent(CircleEvents, arcIndex - 1);
                        if (arcIndex < Arcs.Count)
                            checkCircleEvent(CircleEvents, arcIndex);
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < Arcs.Count; i++)
                        {
                            if (doesIntersect(evt.Site.Position, i, out var intersect))
                            {
                                ...
                                checkCircleEvent(CircleEvents, i);
                                checkCircleEvent(CircleEvents, i + 2);
            ...
            private void checkCircleEvent(List<circleEvent> circleEvents, int arcIndex)
            {
